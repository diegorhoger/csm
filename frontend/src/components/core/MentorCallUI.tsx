...
import VoiceCustomizationControls from '../UI/VoiceCustomizationControls';
...
const MentorCallUI: React.FC = () => {
  ...
  // Handle voice options change
  const handleVoiceOptionsChange = (options) => {
    setVoiceOptions({
      ...options,
      userCustomized: true
    });
    ...
    <div className="flex flex-col items-center w-full max-w-2xl mx-auto space-y-2 p-6">
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <h3 style={{ 
            ...
            {speechEmotion && (
              <span style={{
                ...
                {speechEmotion.charAt(0).toUpperCase() + speechEmotion.slice(1)}
              </span>
            )}
          </h3>
          <button
          ...
          >
            {showVoiceControls ? 'Hide Controls' : 'Voice Settings'}
          </button>
        </div>
        
        {showVoiceControls && (
          <VoiceCustomizationControls
